	...
	for _, loc := range locations {
		...
		for _, group := range equipmentGroups {
			...
			log.WithFields(log.Fields{
				...
				"pickup_to":      toDate,
				"page_size":      baseRequest.Criteria.PageSize,
				"hours_old":      baseRequest.Criteria.HoursOld,
				"integration_id": client.IntegrationID,
				"username":       client.Username,
			}).Info("Prepared LoadSearchRequest for Truckstop")

			seenIDs := make(map[string]struct{}, 512)
			pageNumber := 1
			for {
				request := baseRequest
				...
				log.Infof("✅ RAW XML length for equip group %s (page %d): %d bytes", equipmentTypeStr, pageNumber, len(rawXML))
				if err != nil {
					log.WithError(err).Errorf("Truckstop multi-load detail search failed for address %s and equip group %v (page %d)", loc.Address, group, pageNumber)
					break
				}
				...
				log.Infof("✅ Received %d LoadDetails from Truckstop for address %s (Equip Group %v, page %d)", len(details), loc.Address, group, pageNumber)
				...
				for _, load := range details {
					if load.ID != "" {
						if _, exists := seenIDs[load.ID]; exists {
							continue
						}
						seenIDs[load.ID] = struct{}{}
					}
				...
				if len(details) < request.Criteria.PageSize {
					break
				}
				pageNumber++
			}
